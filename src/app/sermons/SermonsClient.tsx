...
export default function SermonsClient({ sermons, podcasts }: { sermons: any[], podcasts: any[] }) {
                    ...
                    <Card className="overflow-hidden hover:shadow-xl transition-all hover:-translate-y-1">
                      <div className="relative">
                        <div
                          ...
                          style={{ backgroundImage: `url(${sermon.thumbnail || 'https://images.unsplash.com/photo-1507537362848-9c7e70b7b5c1?w=800&q=80'})` }}
                        >
                          <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity">
                            <Button size="lg" className="rounded-full w-16 h-16 bg-accent hover:bg-accent/90 text-secondary">
                              <a href={sermon.videoUrl || sermon.url} target="_blank" rel="noopener noreferrer">
                      ...
                      <CardContent className="space-y-3">
                        ...
                        <div className="flex items-center gap-2 text-sm text-muted-foreground">
                          ...
                          <span>{sermon.date ? new Date(sermon.date).toLocaleDateString() : new Date(sermon.createdAt).toLocaleDateString()}</span>
                        </div>
                        <div className="flex items-center justify-between pt-4">
                          <span className="text-sm text-muted-foreground">{sermon.views || 0} views</span>
                  ...
                  >
                    <Card className="overflow-hidden hover:shadow-xl transition-all hover:-translate-y-1">
                      <div className="relative">
                        <div
                          className="h-48 bg-cover bg-center"
                          style={{ backgroundImage: `url(${podcast.thumbnail || 'https://images.unsplash.com/photo-1478737270239-2f02b77fc618?w=800&q=80'})` }}
                        >
                      ...
                      <CardHeader>
                        <p className="text-sm text-muted-foreground mb-2">{podcast.episode || 'Episode'}</p>
                      ...
                      <CardContent className="space-y-3">
                        <div className="flex items-center justify-between text-sm text-muted-foreground">
                          <div className="flex items-center gap-2">
                            <Calendar className="w-4 h-4" />
                            <span>{new Date(podcast.createdAt || Date.now()).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>
                          </div>
                          <span>{podcast.duration || '30:00'}</span>
                        </div>
